...
public class Round {
    ...
    public enum Phase {
        PLAYER_TURN, WAITING_FOR_CLICK, ENDED
    } // enums for all possible phases of a round
    ...
    public boolean isOver() {
        return phase == Phase.ENDED || player.getHealth() <= 0 || enemy.getHealth() <= 0; // returns isOver as true if
                                                                                          // player or enemy health is
                                                                                          // equal to 0 or if phase =
                                                                                          // ended
    }
    ...
    public void onPlayerClickPowerAttack() { // executes when Attack button is clicked
        ...
        if (powerAttack.missed()) {
            ...
            pushEnemyHP();
            pushMessage(player.getName() + " power attacked " + enemy.getName() + " for " + dmg + " damage!"
                    + powerAttack.critMesssage());
        }

        new javax.swing.Timer(1500, e -> { //sets time between moves
            if (enemy.getHealth() <= 0) {
                phase = Phase.ENDED;
                pushMessage(enemy.getName() + " is defeated!");
                notifyEnded(true);
    ...
    public String getEnemySpritePath() {
        return "src/images/" + enemy.getName().toLowerCase() + ".png";
    }
}
